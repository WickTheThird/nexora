          ...
          <div className="grid lg:grid-cols-4 gap-4">
            ...
            >
              {filterPay(paidItems).map((p) => (
                <PaymentCard key={p.id} payment={p} expanded={expanded.has(p.id)} onToggle={() => toggleExpand(p.id)} />
              ))}
            </Column>
          </div>
        </>
...
function Column({
  ...
  return (
    <div className={`rounded-lg border ${t.border} ${t.bg} flex flex-col`}>
      <div className="px-3 py-2.5 border-b border-ink-200 flex items-center justify-between">
...
function OpenCard({
  ...
  return (
    <div className="bg-white rounded-md border border-ink-200 shadow-sm">
      <button
        type="button"
        onClick={onToggle}
        className="w-full text-left px-3 py-2 hover:bg-ink-50/60 rounded-t-md flex items-start justify-between gap-2"
      >
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t border-ink-100 pt-2 text-xs text-ink-700 space-y-1.5">
